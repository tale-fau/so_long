...
/*   Updated: 2021/08/13 15:39:36 by tale-fau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_player(int y, int x, t_data *data)
{
	...
	b = 0;
	while (y + b <= y + data->size_game - 1)
	{
		a = 0;
		while (x + a <= x + data->size_game - 1)
		{
...
int	color_image(t_data *data)
{
	...
	while (data->map[i])
	{
		...
		{
			if (data->map[i][j] == '1')
				draw_walls(i * data->size_game, j * data->size_game, data);
			else if (data->map[i][j] == '0' || data->map[i][j] == 'P')
				draw_playgd(i * data->size_game, j * data->size_game, data);
			else if (data->map[i][j] == 'C')
				draw_utils(i * data->size_game, j * data->size_game, data, 'C');
			else if (data->map[i][j] == 'E')
				draw_utils(i * data->size_game, j * data->size_game, data, 'E');
			j++;
		}
		i++;
	}
	...
	draw_player(data->player_pos[0] * data->size_game, data->player_pos[1]
		* data->size_game, data);
	return (0);
}
